...
const TambahSuratKeluar = ({ isVisible, onClose , user}) => {
    const [createdBy, setCreatedBy] = useState(user ? user.name : ""); // Menggunakan user.name sebagai nilai awal
  const [tanggalSuratKeluar, setTanggalSuratKeluar] = useState("");
  const [noUrutSuratKeluar, setNoUrutSuratKeluar] = useState("");
  ...
  const saveSuratKeluar = async (e) => {
    e.preventDefault();
    try {
      await axios.post("http://localhost:5000/surat_keluar", {
        createdBy: user.name,
        tanggal_surat_keluar: tanggalSuratKeluar,
        no_urut_surat_keluar: noUrutSuratKeluar,
      ...
      setMsg("Surat keluar berhasil ditambahkan!");
      
      // Reset form setelah sukses
      setTanggalSuratKeluar("");
      setNoUrutSuratKeluar("");
      setNoReferensiSurat("");
      setPeruntukanSurat("");
      setPerihalSurat("");
      setPembawaSurat("");
  ...
  return (
    <div className='fixed inset-0 bg-black bg-opacity-25 backdrop-blur-sm flex justify-center items-center'>
      <div className="bg-white p-8 rounded-lg shadow-md w-[100vh]">
        <h2 className="text-2xl font-semibold mb-4">Tambah Surat Keluar</h2>
        {msg && <div className="text-green-500 mb-4">{msg}</div>}
        <form onSubmit={saveSuratKeluar}>
          ...
          <div className="mb-4 w-1/4">
            ...
            <div className="mb-4 w-1/4">
              ...
              <input type="date" id="tanggalSuratKeluar" name="tanggalSuratKeluar" value={tanggalSuratKeluar} className="ml-2 px-3 py-2 border border-slate-400 rounded-md focus:outline-none focus:border-blue-500" onChange={(e) => setTanggalSuratKeluar(e.target.value)} />
            </div>
            <div className="mb-4 w-1/5">
              <label htmlFor="noUrutSuratKeluar" className=" block text-gray-700 font-medium mb-2">No</label>
              <input type="text" id="noUrutSuratKeluar" name="noUrutSuratKeluar" value={noUrutSuratKeluar} className="w-full px-3 py-2 border border-slate-400 rounded-md focus:outline-none focus:border-blue-500" onChange={(e) => setNoUrutSuratKeluar(e.target.value)} />
            </div>
            <div className="mb-4 w-1/5">
